...
class SegFormer3D(nn.Module):
    def __init__(
        self,
        in_channels: int = 1,  # Changed from 4 to 1 for MRI data
    ...
    def forward(self, x):
        # Get input spatial dimensions before processing
        _, _, d, h, w = x.shape
...
class PatchEmbedding(nn.Module):
    def __init__(
        self,
        in_channel: int = 1,  # Changed from 4 to 1
        embed_dim: int = 768,
        kernel_size: int = 7,
        stride: int = 4,
        padding: int = 3,
    ...
    def forward(self, x):
        # Keep track of input spatial dimensions
        _, _, d, h, w = x.shape
        ...
        # Store the spatial dimensions in the patches tensor for later use
        patches.out_dims = (out_d, out_h, out_w)
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        # Extract spatial dimensions if available
        if hasattr(x, 'spatial_dims'):
            d, h, w = x.spatial_dims
        else:
            # Fallback to estimating dimensions based on sequence length
            B, N, C = x.shape
            # Estimate dimensions - this is an approximation
            vol = N
            # Try to find factors close to original aspect ratio
            w = int(round(vol ** (1/3)))
            h = w
            d = int(vol / (h * w))
            # Adjust to make sure d*h*w = N
            while d * h * w > N:
                if d > 1: d -= 1
                elif h > 1: h -= 1
                elif w > 1: w -= 1
        ...
        if self.sr_ratio > 1:
            ...
            # Store new spatial dimensions
            if hasattr(x_, 'spatial_dims'):
                sr_d = d // self.sr_ratio
                sr_h = h // self.sr_ratio
                sr_w = w // self.sr_ratio
                if sr_d * sr_h * sr_w != x_.shape[1]:
                    # Fallback - recalculate dimensions
                    sr_vol = x_.shape[1]
                    sr_w = int(round(sr_vol ** (1/3)))
                    sr_h = sr_w
                    sr_d = int(sr_vol / (sr_h * sr_w))
                x_.spatial_dims = (sr_d, sr_h, sr_w)
        ...
        if hasattr(x, 'spatial_dims'):
            out.spatial_dims = x.spatial_dims
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x):
        # Preserve spatial dimensions through operations
        attn_out = self.attention(self.norm1(x))
        if hasattr(x, 'spatial_dims'):
            attn_out.spatial_dims = x.spatial_dims
            
        x = x + attn_out
        
        mlp_out = self.mlp(self.norm2(x))
        if hasattr(x, 'spatial_dims'):
            mlp_out.spatial_dims = x.spatial_dims
...
class MixVisionTransformer(nn.Module):
    def __init__(
        self,
        in_channels: int = 1,  # Changed from 4 to 1
    ...
    def forward(self, x):
        ...
        # Stage 1: First embedding and transformer blocks
        x1 = self.embed_1(x)
        B, N, C = x1.shape
        
        # Get output spatial dimensions from convolution
        d1, h1, w1 = x1.out_dims if hasattr(x1, 'out_dims') else get_dim_from_tensor(x1, B, C)
        
        # Store spatial dimensions for attention blocks
        x1.spatial_dims = (d1, h1, w1)
        ...
        # Reshape to 3D volume for next stage
        x1_3d = x1.reshape(B, d1, h1, w1, -1).permute(0, 4, 1, 2, 3).contiguous()
        out.append(x1_3d)

        # Stage 2: Second embedding and transformer blocks
        x2 = self.embed_2(x1_3d)
        B, N, C = x2.shape
        
        # Get output dimensions
        d2, h2, w2 = x2.out_dims if hasattr(x2, 'out_dims') else get_dim_from_tensor(x2, B, C)
        
        # Store spatial dimensions
        x2.spatial_dims = (d2, h2, w2)
        ...
        x2_3d = x2.reshape(B, d2, h2, w2, -1).permute(0, 4, 1, 2, 3).contiguous()
        out.append(x2_3d)

        # Stage 3: Third embedding and transformer blocks
        x3 = self.embed_3(x2_3d)
        B, N, C = x3.shape
        
        # Get output dimensions
        d3, h3, w3 = x3.out_dims if hasattr(x3, 'out_dims') else get_dim_from_tensor(x3, B, C)
        
        # Store spatial dimensions
        x3.spatial_dims = (d3, h3, w3)
        ...
        # Reshape for next stage
        x3_3d = x3.reshape(B, d3, h3, w3, -1).permute(0, 4, 1, 2, 3).contiguous()
        out.append(x3_3d)

        # Stage 4: Fourth embedding and transformer blocks
        x4 = self.embed_4(x3_3d)
        B, N, C = x4.shape
        
        # Get output dimensions
        d4, h4, w4 = x4.out_dims if hasattr(x4, 'out_dims') else get_dim_from_tensor(x4, B, C)
        
        # Store spatial dimensions
        x4.spatial_dims = (d4, h4, w4)
        ...
        # Reshape for output
        x4_3d = x4.reshape(B, d4, h4, w4, -1).permute(0, 4, 1, 2, 3).contiguous()
        out.append(x4_3d)
...
class _MLP(nn.Module):
    ...
    def forward(self, x):
        # Preserve spatial dimensions through MLP
        spatial_dims = x.spatial_dims if hasattr(x, 'spatial_dims') else None
...
class DWConv(nn.Module):
    ...
    def forward(self, x):
        ...
        if hasattr(x, 'spatial_dims'):
            ...
            d, h, w = get_dim_from_tensor(x, B, C)
        ...
        # Preserve spatial dimensions
        x.spatial_dims = (d, h, w)
        
        return x

###################################################################################
def get_dim_from_tensor(tensor, batch_size, channels):
    # Helper function to determine spatial dimensions from flattened tensor
    num_elements = tensor.shape[1]
    
    # Try to find factors close to typical medical image aspect ratios
    # Medical images often have depth < height ≈ width
    total_vol = num_elements
    
    # Start with a cube estimate
    w = int(round(total_vol ** (1/3)))
    h = w
    d = total_vol // (h * w)
    
    # Adjust to ensure d*h*w = num_elements
    while d * h * w > num_elements:
        if d > 1: d -= 1
        elif h > 1: h -= 1
        elif w > 1: w -= 1
