...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.sr_ratio > 1:
            ...
            # n = cube_root(N)
            # Use:
            d, h, w = get_dimensions(N, original_shape=x.shape)
            print(f"d = {d}, h = {h}, w = {w}")
...
def get_dimensions(n, original_shape):
    """Calculate dimensions for reshaping based on original shape ratios"""
    total = n
    original_d, original_h, original_w = original_shape
    
    # Find the approximate cube root
    approx_size = round(math.pow(total, 1/3))
    
    # Calculate ratios from original shape
    d_ratio = original_d / original_w
    h_ratio = original_h / original_w
    
    # Apply ratios to calculate dimensions
    w = approx_size
    h = round(w * h_ratio)
    d = round(w * d_ratio)
    
    # Fine-tune to ensure d*h*w = total
    while d*h*w > total:
        if d > 1: d -= 1
        elif h > 1: h -= 1
        elif w > 1: w -= 1
