...
from models.cbam_3d import CBAM3D
...
# Define Multiclass 3D U-Net architecture with CBAM
class UNet3DMulticlassCBAM(nn.Module):
    def __init__(self, in_channels, out_channels, num_kernels_1, r=4):
        """
        Initialize UNet3DMulticlassCBAM model.
        Args:
            in_channels: Number of input channels
            out_channels: Number of output channels
            num_kernels_1: Number of kernels in the first encoding block
            r: Reduction ratio for channel attention
        """

        super(UNet3DMulticlassCBAM, self).__init__()
        ...
        self.cbam3 = CBAM3D(num_kernels_1*4, r=self.r)
        self.dec2 = UpConvBlock(num_kernels_1*4, num_kernels_1*2)
        self.cbam2 = CBAM3D(num_kernels_1*2, r=self.r)
        self.dec1 = UpConvBlock(num_kernels_1*2, num_kernels_1)
        self.cbam1 = CBAM3D(num_kernels_1, r=self.r)
