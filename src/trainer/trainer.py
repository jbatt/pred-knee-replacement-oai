...
def train_patch_loop(
    ...
    for batch, (X_volume, y_volume) in enumerate(dataloader):
        ...
        # If using patches combine patches into batch dimension (batch_size * num_patches, channels, patch_size[0], patch_size[1], patch_size[2])
        # Accumulate gradients over all patches in volume
        
        # TODO: remove harcoded nmber of channels as 1
        if patch_method == "grid_pytorch" and patch_size is not None:
            ...
            # TODO: have this patch loop be for the  grid_pytorch method only 
        ...
        # For random_monai approach just use normal dataloader / training loop
        # Gradients are not accumulated
        elif patch_method == "random_monai" and patch_size is not None:
        ...
        # Perform model step - gradients have been accumulated over patch sub-batch if using grid_pytorch patching method
        scaler.step(optimizer)
        scaler.update()            
